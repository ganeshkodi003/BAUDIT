...
public interface paystructurerep extends JpaRepository<paystructureentity, String>{
	@Query(value = "SELECT * FROM BTM_SAL_PAY WHERE del_flg='N' AND emp_no IN (SELECT DISTINCT emp_no FROM BAJ_SAL_PAY) ORDER BY emp_no", nativeQuery = true)
	...
	paystructureentity getpaystructureedit(String emp_no,String salaryMonth,String empname);
	 ...
	 @Query(value = "SELECT * FROM BTM_SAL_PAY WHERE del_flg='N' AND salary_month=?1", nativeQuery = true)
	    ...
	    @Query(value = "SELECT * FROM BTM_SAL_PAY WHERE del_flg='N' AND SUBSTR(salary_month, 1, 4) = ?1 AND SUBSTR(salary_month, 5, 2) = ?2", nativeQuery = true)
