...
public interface BAJ_TrmView_Repo extends JpaRepository<BAJ_TrmView_Entity, BAJ_TrmView_Entity_Idclass>,BAJ_TrmView_RepoCustom {
	...
	@Query(value = "Select * From TRMWORK_VIEW Where acct_num =?1 AND TRAN_DATE >= TO_DATE('01-04-2024', 'DD-MM-YYYY') AND TRAN_DATE <= TO_DATE('01-04-2025', 'DD-MM-YYYY') ORDER BY TRAN_DATE,TRAN_ID,PART_TRAN_ID,PART_TRAN_TYPE ASC ", nativeQuery = true)
	...
	@Query(value = "SELECT * " + "FROM TRMWORK_VIEW " + "WHERE tran_id IN (" + "    SELECT DISTINCT tran_id "
			...
			+ "ORDER BY TRAN_DATE,TRAN_ID,PART_TRAN_ID,PART_TRAN_TYPE ASC", nativeQuery = true)
	...
	@Query(value = "SELECT * FROM TRMWORK_VIEW WHERE ACCT_NUM = ?1 AND TRUNC(TRAN_DATE) BETWEEN TO_DATE(?2, 'dd-MM-yyyy') AND TO_DATE(?3, 'dd-MM-yyyy') ORDER BY TRAN_DATE,TRAN_ID,PART_TRAN_ID,PART_TRAN_TYPE", nativeQuery = true)
	List<BAJ_TrmView_Entity> getTranlst2(String acct_num, String fromdate, String todate);

	@Query(value = "SELECT * FROM TRMWORK_VIEW WHERE ACCT_NUM IN (:acctNums) AND TRUNC(TRAN_DATE) BETWEEN TO_DATE(:fromDate, 'dd-MM-yyyy') AND TO_DATE(:toDate, 'dd-MM-yyyy') ORDER BY TRAN_DATE,TRAN_ID,PART_TRAN_ID,PART_TRAN_TYPE", nativeQuery = true)
	...
	@Query(value = "SELECT * " + "FROM TRMWORK_VIEW " + "WHERE tran_id IN (" + "    SELECT DISTINCT tran_id "
			...
			+ "AND TRAN_DATE BETWEEN TO_DATE(?2, 'dd-MM-yyyy') AND TO_DATE(?3, 'dd-MM-yyyy') ORDER BY TRAN_DATE,TRAN_ID,PART_TRAN_ID,PART_TRAN_TYPE ASC", nativeQuery = true)
