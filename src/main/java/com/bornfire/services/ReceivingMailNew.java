...
public class ReceivingMailNew {
	...
	public String receiveEmail(String pop3Host, String storeType, String user, String password,
			...
			PlacementMaintenance placementmaintenance) throws InterruptedException {
		POP3Store emailStore = null;
        Folder emailFolder = null;
    	Properties properties = new Properties();

		properties.put("mail.pop3.host", pop3Host);
		properties.put("mail.pop3.port", "995"); // Use port 995 for secure POP3
		...
		try {
			...
			// Acquire a connection permit (blocks if all 10 permits are in use)
			...
			if (S1.equals("FW: ECM") || S1.equals("Fw: ECM") || S3.equals("Fwd: ECM") ) {
				...
				for (int i = 0; i < Lmessages.length; i++) {
					...
					/*
					 * String states = ss.substring(ss.indexOf("Date: "),
					 * ss.indexOf("To: \"siddhaiyan@bornfire.in\" <siddhaiyan@bornfire.in>"));
					 * 
					 * System.out.println(states); String Adate;
					 * 
					 * if (states.contains("day, ")) { // Extract date when the day of the week is
					 * present Adate = states.substring(states.indexOf("day, "));
					 * System.out.println(Adate + " Adate with day"); } else { // Extract date when
					 * the day of the week is NOT present Adate =
					 * states.substring(states.indexOf("Sent: ") + 6); System.out.println(Adate +
					 * " Adate without day");
					 * 
					 * // Normalize Adate to include ", 20" for consistency if
					 * (!Adate.contains(", 20")) { // Convert "07 N ovember 2024 15:01" to
					 * "November 7, 2024" String[] parts = Adate.split(" "); if (parts.length >= 3)
					...
					int toIndex = ss.indexOf("To: \"siddhaiyan@bornfire.in\" <siddhaiyan@bornfire.in>");

					if (dateIndex != -1 && toIndex != -1 && dateIndex < toIndex) {
					    String states = ss.substring(dateIndex, toIndex);
					    System.out.println(states);
					    
					    String Adate;

					    if (states.contains("day, ")) {
					        Adate = states.substring(states.indexOf("day, ") + 5); 
					        System.out.println(Adate + " Adate with day");
					    } else {
					        Adate = states.substring(dateIndex + 6);
